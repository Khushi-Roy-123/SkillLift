...
import AskSkillLiftPage from './pages/AskSkillLiftPage';
import OpportunitiesPage from './pages/OpportunitiesPage';
import InspirationHubPage from './pages/InspirationHubPage';
import SkillQuestPage from './pages/SkillQuestPage';
...
const App: React.FC = () => {
  ...
  useEffect(() => {
    const init = async () => {
      ...
      if (storedUser) {
        ...
        // Migration: Ensure new properties exist for existing users
        if (!parsedUser.tasks) parsedUser.tasks = [];
        if (!parsedUser.hydration) {
            parsedUser.hydration = {
                count: 0,
                goal: 8,
  ...
  const handleLogin = (user: User) => {
    ...
    if (!user.hydration) {
        user.hydration = { count: 0, goal: 8, lastUpdated: new Date().toISOString() };
    }
    if (!user.tasks) {
        user.tasks = [];
    }
  ...
  if (isInitializing) {
    return (
        <div className="flex flex-col items-center justify-center h-screen bg-sky-50 text-gray-800">
            ...
            <p className="mt-4 text-lg font-semibold">Loading SkillLift...</p>
        </div>
    );
  }
  ...
  const renderPage = () => {
    switch (activePage) {
      ...
      case 'SkillQuest':
        return <SkillQuestPage currentUser={currentUser} onUpdateUser={handleUpdateUser} />;
      case 'Wellbeing Hub':
        return <WellbeingHubPage currentUser={currentUser} onUpdateUser={handleUpdateUser} />;
      case 'Life Balance':
        return <LifeBalancePage currentUser={currentUser} onUpdateUser={handleUpdateUser} />;
      case 'Resume Tips':
