...
export const askSkillLift = async (history: ChatMessage[], newMessage: string): Promise<string> => {
  try {
    ...
    logError(error, { function: 'askSkillLift' });
...
export const getMealSuggestion = async (ingredients: string, time: string): Promise<string> => {
    ...
    try {
        ...
        logError(error, { function: 'getMealSuggestion' });
        return "I'm sorry, I couldn't generate a meal plan right now. How about a simple stir-fry or a salad?";
    }
};

export const getLifeHack = async (category: string): Promise<string> => {
    const systemInstruction = `You are a life-hack guru specializing in work-life balance for professionals. Provide one actionable, specific, and high-impact tip. Be brief and encouraging.`;
    
    const userPrompt = `Give me a life hack for: ${category}.`;
    ...
    try {
        ...
        return response.text;
         ...
         console.error("Error getting life hack:", error);
        logError(error, { function: 'getLifeHack' });
        return "Tip: Break large tasks into 5-minute micro-tasks to get started effortlessly.";
    }
};
