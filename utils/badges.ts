...
export const ALL_BADGES: Badge[] = [
    ...
    {
        id: 'skillquest-champion',
        name: 'SkillQuest Champion',
...
export const checkForNewBadges = (user: User, allCareerPaths: CareerPath[]): string[] => {
    ...
    // Badge: SkillQuest Champion (complete one career path)
    if (!userBadges.has('skillquest-champion') && user.selectedCareerPath) {
        ...
        if (path) {
            ...
            if (totalQuestsInPath > 0 && completedInPath === totalQuestsInPath) {
                newBadges.push('skillquest-champion');
            }
        }
    }
